	...
	<div class="fixed top-0 left-0 flex w-full h-full justify-center items-center overflow-hidden">
		...
		>
			...
			<div class="flex flex-col gap-2">
				<div
					class="border-2 border-main p-0.5 flex flex-col bg-gradient-to-r from-indigo-600 to-indigo-800 text-white"
				>
					<div class="p-7 rounded-md">
						<div class="font-bold sm:text-xl text-main uppercase">
							<slot name="header" />
						</div>
						<HorizontalBorder />
						<slot name="description" />
					</div>
					<div class="pl-1 text-xs text-main">
						<p>!*^$#^&?@_</p>
					</div>
				</div>
