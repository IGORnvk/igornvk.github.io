...
{#each treeSettings as tree}
    <Tree scale={tree.scale} position.x={tree.x} position.y={tree.y} position.z={tree.z} />
{/each}
...
<SmallRock scale={0.4} position.x={0.4} position.y={$heightForItems} position.z={0.2} />


<FishingStand position.x={0.1} position.y={$heightForItems} position.z={0.4} rotation.y={1} />

<WoodenSign scale={0.2} position.x={-0.1} position.y={$heightForItems} rotation.y={-0.9} />
